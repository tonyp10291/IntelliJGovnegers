// src/main/java/kr/co/govengers/service/QnaService.java
...
public class QnaService {
    ...
    /**
     * 컨트롤러에서 사용하는 시그니처와 동일:
     * addAdminComment(qid, content, auth.getName())
     * - auth.getName()이 uid가 아닐 수도 있어 fallback 처리 포함
     */
    @Transactional
    public Long addAdminComment(Long qid, String content, String adminUidMaybeName) {
        ...
        // 1) 서비스 레벨에서도 ROLE_ADMIN 확인(2중 방어)
        ...
        // 2) adminUid 보정: JwtFilter에서 principal=User 이므로 getName() 대신 principal의 uid 사용
    ...
    /** DTO를 반환하는 버전(다른 컨트롤러에서 사용할 수 있음) */
    @Transactional
    ...
    public void delete(Long qid, String requesterUid) {
        ...
        // ★ 여기서 SecurityException 말고 AccessDeniedException 던지기 → 403
