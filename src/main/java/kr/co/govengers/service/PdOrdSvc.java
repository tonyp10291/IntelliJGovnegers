...
public class PdOrdSvc {
    ...
    private Map<String, Object> convertOrderToFrontendFormat(OrderInfo order) {
        ...
        m.put("impUid", order.getImpUid()); // 취소용 추가
        ...
        List<Map<String, Object>> items = orderItems.stream().map(oi -> {
            ...
            im.put("id", oi.getId());
            im.put("orderItemId", oi.getId()); // 취소용 추가
            im.put("pnm", oi.getPnm());
            im.put("name", oi.getPnm()); // ✅ 프론트에서 item.name으로 참조
            im.put("quantity", quantity);
            im.put("price", unitPrice);

            // ✅ 프론트엔드에서 필요한 필드들 추가
            im.put("unitPrice", unitPrice);     // 단가 (프론트에서 사용)
            im.put("subtotal", subtotal);       // 소계 (프론트에서 사용)
            im.put("totalPrice", subtotal);     // 기존 호환성
